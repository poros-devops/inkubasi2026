    ...
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 z-50 w-full max-w-md bg-[#FAFAF8] shadow-2xl flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-[#E8E6DF]">
          ...
          <button
            onClick={toggleCart}
            className="p-2 hover:bg-[#F1EFE8] rounded-full"
          >
          ...
          {!user ? (
            ...
            <div className="space-y-4">
              {cart.items.map((item) => (
                <div
                  key={item.id}
                  className="flex gap-4 py-4 border-b border-[#E8E6DF] last:border-0"
                >
                  {/* Image placeholder */}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[10px] text-[#888780] tracking-widest uppercase">
                      {item.product.brand}
                    </p>
                    ...
                    <div className="flex items-center justify-between mt-3">
                      <div className="flex items-center gap-2 border border-[#E8E6DF] rounded-full px-3 py-1">
                        <button
                          ...
                          className="text-[#888780] hover:text-[#1C1C1A]"
                        >
                          <Minus size={12} />
                        </button>
                  ...
                  <button
                    ...
                    className="text-[#C4C2BA] hover:text-[#A32D2D] self-start mt-1"
                  >
                    <X size={14} />
                  </button>
                </div>
        ...
        {cart?.items && cart.items.length > 0 && (
          <div className="px-6 py-5 border-t border-[#E8E6DF] space-y-4">
            ...
            <button
              onClick={toggleCart}
              className="block w-full text-center text-sm text-[#888780] hover:text-[#1C1C1A]"
            >
              Continue Shopping
            </button>
          </div>
