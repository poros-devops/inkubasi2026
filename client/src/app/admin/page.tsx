    ...
    return (
      <div className="pt-24 px-6 max-w-7xl mx-auto animate-pulse">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="h-24 bg-[#F1EFE8] rounded-2xl" />
          ))}
        </div>
      </div>
    );
    ...
    <div className="pt-24 pb-16 min-h-screen">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <p className="text-xs text-[#888780] tracking-widest uppercase mb-1">
              Admin Panel
            </p>
            <h1 className="font-display text-4xl font-light text-[#1C1C1A]">
              Dashboard
            </h1>
          </div>
          <Link
            href="/products/new"
            className="hidden text-sm bg-[#1C1C1A] text-white px-5 py-2.5 rounded-full hover:bg-[#2C2C2A] transition-colors"
          >
            + Add Product
          </Link>
        </div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              {[
                {
                  label: 'Revenue',
                  value: f(dashboard.revenue?.total || 0),
                  sub: `${dashboard.revenue?.count || 0} orders`,
                },
                {
                  label: 'Total Orders',
                ...
                {
                  label: 'Products',
                  value: dashboard.productCount?.toLocaleString(),
                  sub: 'Active',
                },
                {
                  label: 'Customers',
                  value: dashboard.customerCount?.toLocaleString(),
                  sub: 'Registered',
                },
              ].map((s) => (
                <div key={s.label} className="bg-[#F1EFE8] rounded-2xl p-5">
                  <p className="text-xs text-[#888780] mb-2">{s.label}</p>
                  <p className="text-xl font-medium text-[#1C1C1A]">
                    {s.value}
                  </p>
            ...
            <div className="bg-white border border-[#E8E6DF] rounded-2xl overflow-hidden">
              <div className="px-6 py-4 border-b border-[#E8E6DF] flex justify-between items-center">
                ...
                <button
                  ...
                  className="text-xs text-[#888780] hover:text-[#1C1C1A]"
                >
                  View all
                </button>
              </div>
                ...
                <table className="w-full text-sm">
                    ...
                    <tr className="border-b border-[#E8E6DF]">
                      {['Order', 'Customer', 'Total', 'Status', 'Date'].map(
                        (h) => (
                          <th
                            key={h}
                            className="text-left px-6 py-3 text-xs text-[#888780] font-normal"
                          >
                            {h}
                          </th>
                        )
                  ...
                  <tbody>
                    {orders.slice(0, 5).map((o) => (
                      <tr
                        key={o.id}
                        className="border-b border-[#E8E6DF] last:border-0 hover:bg-[#FAFAF8]"
                      >
                        <td className="px-6 py-4 font-medium text-xs">
                          #{o.orderNumber}
                        </td>
                        <td className="px-6 py-4 text-xs text-[#5F5E5A]">
                          {o.user?.name || '—'}
                        </td>
                        <td className="px-6 py-4 text-xs">{f(o.total)}</td>
                        <td className="px-6 py-4">
                          <span
                            className={`text-[10px] font-medium px-2.5 py-1 rounded-full capitalize ${STATUS_STYLES[o.status]}`}
                          >
                            {o.status}
                          </span>
                        </td>
                        <td className="px-6 py-4 text-xs text-[#888780]">
          ...
          <div className="bg-white border border-[#E8E6DF] rounded-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-[#E8E6DF]">
              <h2 className="font-medium text-sm">
                All Orders{' '}
                <span className="text-[#888780] font-normal">
                  ({orders.length})
                </span>
              </h2>
            </div>
                ...
                <tbody>
                  {orders.map((o) => (
                    <tr
                      ...
                      className="border-b border-[#E8E6DF] last:border-0 hover:bg-[#FAFAF8]"
                    >
                      <td className="px-6 py-4 font-medium text-xs text-[#1C1C1A]">
                        #{o.orderNumber}
                      </td>
                      <td className="px-6 py-4 text-xs text-[#5F5E5A]">
                        {o.user?.name || '—'}
